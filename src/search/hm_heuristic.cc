...
void HMHeuristic::init_hm_table(hm_tuple &t) {
    map<hm_tuple, int>::iterator it;
    for (it = hm_table.begin(); it != hm_table.end(); it++) {
        pair<hm_tuple, int> hm_ent = *it;
        hm_tuple &tup = hm_ent.first;
...
void HMHeuristic::extend_tuple(hm_tuple &t, const Operator &op) {
    map<hm_tuple, int>::const_iterator it;
    for (it = hm_table.begin(); it != hm_table.end(); it++) {
        pair<hm_tuple, int> hm_ent = *it;
        hm_tuple &entry = hm_ent.first;
...
int HMHeuristic::eval(hm_tuple &t) {
    vector<hm_tuple> partial;
...
int HMHeuristic::update_hm_entry(hm_tuple &t, int val) {
...
void HMHeuristic::generate_all_tuples(int var, int sz, hm_tuple &base) {
    if (sz == 1) {
...
void HMHeuristic::generate_all_partial_tuple(hm_tuple &base_tuple, hm_tuple &t,
                                             int index, int sz, vector<hm_tuple> &res) {
    if (sz == 1) {
...
int HMHeuristic::check_tuple_in_tuple(const hm_tuple &tup, const hm_tuple &big_tuple) {
