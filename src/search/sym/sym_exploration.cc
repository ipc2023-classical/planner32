...
void SymExploration::closeStates(Bucket & bucket, int g_val) {
...
void SymExploration::checkCutOriginal(Bucket & bucket, int g_val){
...
BDD SymExploration::compute_heuristic(const BDD & from, int fVal, int hVal, bool store_eval){
...
bool SymExploration::isUseful(const vector<BDD> & evalStates,
...
bool SymExploration::isBetter(const SymExploration & other) const{
    return f > other.f || (f == other.f && nextStepTime() < other.nextStepTime());
}
