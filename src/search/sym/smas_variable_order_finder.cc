#include "smas_variable_order_finder.h"
...
void SMASVariableOrderFinder::select_next(int position, int var_no) {
...
bool SMASVariableOrderFinder::done() const {
    return remaining_vars.empty();
}

int SMASVariableOrderFinder::next() {
    assert(!done());
