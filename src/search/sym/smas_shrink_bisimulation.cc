...
SMASShrinkBisimulation::SMASShrinkBisimulation(const Options &opts)
...
SMASShrinkBisimulation::~SMASShrinkBisimulation() {
}

string SMASShrinkBisimulation::name() const {
    return "bisimulation";
}

void SMASShrinkBisimulation::dump_strategy_specific_options() const {
...
bool SMASShrinkBisimulation::reduce_labels_before_shrinking() const {
    return true;
}

bool SMASShrinkBisimulation::shrink(
    ...
    if (must_shrink(abs, min(target, threshold), force)) {
        ...
        relaxed = compute_abstraction(abs, target, equivalence_relation);
        apply(abs, equivalence_relation, target);
    }
    return relaxed;
}

void SMASShrinkBisimulation::shrink_atomic(SymSMAS &abs) {
    ...
    SMASShrinkStrategy *strategy = create_default();
...
bool SMASShrinkBisimulation::shrink_before_merge(
    SymSMAS &abs1, SymSMAS &abs2) {
  pair<int, int> new_sizes = compute_shrink_sizes(abs1.size(), abs2.size(), 
    ...
    return shrink(abs1, new_size1);
}

int SMASShrinkBisimulation::initialize_groups(const SymSMAS &abs,
...
void SMASShrinkBisimulation::compute_signatures(
    const SymSMAS &abs,
...
bool SMASShrinkBisimulation::compute_abstraction(
    SymSMAS &abs,
    int target_size,
    EquivalenceRelation &equivalence_relation) {
    ...
    bool relaxed = (num_groups < num_states) &&
      (greedy || !stable || stop_requested);
...
static Plugin<SMASShrinkStrategy> _plugin("shrink_bisimulation", _parse);
